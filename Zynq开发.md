...
## AXI DMA
- 字节类型定义：数据字节、位置字节（占位符）、空字节，由TSTRB和TKEEP端口指定
- 字节流包含数据字节和空字节，空字节适用于带宽优化或时序调整，例如视频消隐区间填充无效数据
- 连续对齐流只有数据字节，用于高效传输连续数据块
- 连续不对齐流包含数据字节和位置字节，位置字节一般出现在开头或者末尾，用于处理非整数倍总线宽度的数据流，如音频数据流
- 稀疏流包含数据字节和位置字节，通过混合二者，解决了非连续数据的高效传输问题，适合需要保留数据位置信息的场景，如激光雷达点云数据中，无效点用位置字节标记
- 先开启axi dma接收，在产生数据进行传输。这样可以提高带宽，同时防止dma截流机制导致数据丢失
- 使用动态内存malloc函数时，注意分配内存上限，数据量过大可修改
## 自定义IP核
- 封装IP核的设计文件可以包含官方IP核，但是需要在工程文件中手动添加XCI文件（定制IP核产生的文件，其中包含了IP核的所有详细信息）
- 带双向引脚的IO需要做特殊处理，不做处理IO只能作为输出，不可以作为输入。inout类型拆成三个信号，选择接口为GPIO_RTL，此时注意GPIO的控制信号0是输出，1是输入。与IIC中的控制信号相反，连续赋值时需要取反。
- 封装接口可以自定义接口，详见教程。
## AXI4-Stream Video
